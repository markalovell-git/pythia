...
## Framework & Licensing
- [ ] **Convert the frontend from PyQt6 to PySide6** — PyQt6 (Riverbank) is **GPL v3 or paid commercial license only** (no LGPL option), so a closed-source *sellable* build of Pythia would require either open-sourcing the whole app under GPL or buying a commercial PyQt license. **PySide6** is the official Qt-for-Python binding from The Qt Company, wraps the **identical Qt 6 framework** (so zero loss of capability — same widgets, `QPropertyAnimation`, `QGraphicsEffect`, custom `QPainter`, all of it), and is **LGPL v3**, which permits shipping a **closed-source, commercial** product as long as the Qt libs stay replaceable (PyInstaller already does this) and the license notices are included. The PyQt6 → PySide6 API is ~95% identical; the migration is mostly mechanical. Scope to cover: (a) swap `from PyQt6...` imports to `PySide6` across `app/frontend/` (widgets, workers, main, main_window); (b) signal/slot idiom differences — PyQt's `pyqtSignal`/`pyqtSlot` become `Signal`/`Slot`; (c) enum access (both are scoped in Qt6, so largely compatible, but verify); (d) `exec()` vs `exec_()` and any `.toPython()`/`QVariant` edges; (e) update `pyproject.toml` dependency (`pyqt6` → `pyside6`) and the PyInstaller spec/hidden-imports in `packaging/pythia.spec`; (f) re-run the full pytest + pytest-qt suite and do a manual smoke test of every view. Also revisit the `license = "MIT"` field in `pyproject.toml` — currently the distributed AppImage bundles GPL PyQt6, so the shipped work is effectively GPL-bound regardless of the MIT label; PySide6 + LGPL resolves this for a sellable product. (If leadership ever wants *zero* copyleft obligations even under LGPL, the fallback is a Qt commercial license, or a rewrite onto an MIT/BSD toolkit — but PySide6 is the low-effort, full-capability path.)
